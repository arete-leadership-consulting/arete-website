...
export default function InsightsPage() { return <PageShell>
  ...
  <section className="insights-library section"><div className="section-heading"><div><p className="eyebrow">The library</p><p className="section-number">01</p></div><h2>Leadership. Service. Culture. Business. Technology.</h2><p>A growing collection of field notes from the work of leading, building, and serving.</p></div><div className="insights-list">{publishedInsights.map((insight, index) => <article key={insight.slug}><div className={`insight-mark mark-${index + 1}`}><span>0{index + 1}</span></div><div><p>{insight.category}</p><h3>{insight.title}</h3><p>{insight.excerpt}</p><Link className="read-insight" href={`/insights/${insight.slug}`}>Read insight <span>↗</span></Link></div><span className="insight-status published">Published</span></article>)}{futureInsights.map(([category, title, summary], index) => <article key={title}><div className={`insight-mark mark-${(index % 3) + 1}`}><span>0{index + 4}</span></div><div><p>{category}</p><h3>{title}</h3><p>{summary}</p></div><span className="insight-status">Essay in development</span></article>)}</div></section>
